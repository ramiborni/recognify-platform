...
// This modal for creating User and Team after login from Kinde Auth.
const OrgModel = () => {
  ...
  const { mutate: AddUserMutate } = useMutation({
    mutationFn: async ({ token, inviteToken }: AddUserMutationProps) => await addUser(token, inviteToken),
    onSuccess: () => {
