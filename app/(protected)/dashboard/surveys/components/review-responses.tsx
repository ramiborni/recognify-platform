...
const ReviewResponses: React.FC<ReviewResponsesProps> = ({ survey }) => {
  ...
  const getCurrentResponse = () => {
    ...
    const answer = responses.find((a: any) => a.questionId === currentQuestion.id)
    return answer ? answer.userResponse : "No answer provided"
  }
        ...
        return (
          <RadioGroup value={response} className="mt-2 space-y-2">
            {currentQuestion.options.map((option: string, index: number) => (
              ...
              >
                <RadioGroupItem value={option} id={`option-${index}`} className="border-2 border-primary text-primary" />
                <Label htmlFor={`option-${index}`} className="flex-grow cursor-pointer text-sm font-medium">
                  {option}
                </Label>
                {option === response && <Check className="h-4 w-4 text-primary" />}
              </div>
            ))}
          </RadioGroup>
        )
  ...
  return (
    <Card className="mt-8 overflow-hidden bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-800">
      <CardHeader>
        <CardTitle className="text-2xl font-bold text-primary">Survey Responses</CardTitle>
      </CardHeader>
      <CardContent className="p-6">
        <Progress value={progress} className="mb-4 h-2 bg-gray-200 dark:bg-gray-700" />
